...
class RouterHistory {
            ...
            if (path) {
                ...
                if (opts.replace) {
                    history.replaceState(
                        { auth, path, name, display, strict, state },
                        opts.title || "",
                        newPath
                    );
                } else {
    ...
    async _isAuth(auth) {
        try {
            ...
            if (auth == true) {
                ...
                //public route
                return true;
            } else if (Utils.is.isArray(auth)) {
                //role allowed to the route
                let user = this.user;
                if (user) {
                    let role = user.role;
    ...
    async _destroyComponent(components) {
        try {
            ...
            return await Utils.function.recurse(
                components || [],
                async (component, index) => {
                    if (component) {
